...
以上简要介绍了一下内核模式，然后让我们开始聊聊本节的重头戏————上下文。
    4. 上下文切换的触发条件
        - **主动让出**
            1. 使用sleep()等系统调用进入就绪状态。
            2. 使用sched_yield()或pthread_yield()等函数，通知调度器让出CPU。
        - **进程阻塞**
            1. 进程在等待网络请求或外部数据时会进入阻塞状态，此时调度器会选择其他的就绪进程使用CPU,从而避免CPU空闲。
        - **优先级抢占**
             1. 调度器在调度进程是的依据就是为其设置的优先级，若A进程在运行状态时，突然加入了一个更高优先级的进程。此时，调度器将让                 更高优先级的进程使用CPU。 


从上文的触发条件可以看出进程可能因执行某些指令从而主动让出CPU,也有可能因新加入了更高优先级的进程从而别抢占。前者我们或许还可以理解，但后者却有点摸不着头脑，一个进程执行的好好的，调度器是怎么知道新来了一个更高优先级的进程，从而完成调度的？这其实就是设计周期性中断的原因了，有了周期性的中断之后，不管是谁在使用CPU,它会不会主动让出CPU的使用权，操作系统都可以定期的获取CPU的使用权，来重新决定运行哪个进程。  
    5. 上下文的切换过程
        - **切换环境**
            1. 任何的上下文切换都是在内核模式下完成的，
